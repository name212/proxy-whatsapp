...
function exit_with_err() {
  for err_msg in "$@"; do
    echo "[HEALTHCHECKER] $err_msg"
  done
  exit 1
}

if [ -z "$WHATSAPP_STATS_BIND_PORT" ]; then
  WHATSAPP_STATS_BIND_PORT="8199"
fi

stat_url="http://127.0.0.1:${WHATSAPP_STATS_BIND_PORT}/;csv"

raw_result=""

if ! raw_result="$(curl -s -w 2 "$stat_url")"; then
  exit_with_err "Cannot request stats url '$stat_url'" "Raw result from curl: '$raw_result'"
fi
...
result=$(echo "$raw_result" | tail -n +1 | jq -R 'split(",")' | jq -c 'select(.[1] != null) | select(.[1] | contains("whatsapp_net"))' | jq --raw-output '.[65]| select(. | test("Layer4 check passed") | not)')
...
if [ "$result" != "" ]; then
  exit_with_err "Healthchecks failed, L4 healthcheck on *.whatsapp.net failed" "Result: $result"
fi
